...
// preallocating gen and using the same one over multiple times is faster.
// std::vector<int> otherRandomSample(int N, int k, std::mt19937& gen) {
std::vector<int> robertFloydRandomSample(int N, int k) {
  std::vector<int> solution; // (N, 0): preallocating and filling by index is slower.
...
int main() {
  ...
  diff = std::chrono::duration_cast<std::chrono::milliseconds>(end-start);
  printf("execution of SamplePopulatedRange: %lums\n\n", diff.count());
